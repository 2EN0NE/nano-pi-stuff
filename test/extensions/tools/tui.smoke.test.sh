...
# tools 扩展 TUI 端到端测试
# 测试要点：
# 1. /tools 命令在 TUI 模式下正常触发
# 2. tools 的 SettingsList 选择器在 TUI 下不崩溃
# 3. 与 pi-logger 配合正常
# ──────────────────────────────────────────────────────────────────────────────

test_describe "tools extension (TUI mode)"

# ── 用例 1：/tools 在 TUI 模式下触发无崩溃 ──
test_it "/tools command triggers without crash" <<'TEST'
  tui_run_pi_test "pi-logger,tools" "/tools" 15
...
# ── 用例 2：TUI 模式下 tools 扩展加载正常 ──
test_it "shows tools in TUI extension list" <<'TEST'
  tui_run_pi_test "pi-logger,tools" "/tools" 15

  # 验证 pi-logger 和 tools 都在扩展列表中
...
# ── 用例 3：TUI 模式下 /tools 选择器面板正常工作 [REVIEW] ──
test_it "/tools shows selector panel [REVIEW]" <<'TEST'
  tui_run_pi_test "pi-logger,tools" "/tools" 15

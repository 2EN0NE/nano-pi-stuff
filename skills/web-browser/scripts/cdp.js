...
function exceptionMessage(result) {
	return result.exceptionDetails.exception?.description || result.exceptionDetails.text;
}
...
class CDP {
	...
	async evaluateRepl(sessionId, expression, timeout = 30000) {
		...
		try {
			...
			await this.send('Runtime.releaseObjectGroup', { objectGroup }, sessionId, 1000).catch(
				() => {},
			);
		}
	}

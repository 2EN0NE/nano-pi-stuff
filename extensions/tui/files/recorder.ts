...
function extractPathsFromText(text: string): string[] {
	...
	const homeRe = /(?:\s|^|['"\`(])(~\/(?:[^\s"'`<>)}\]/]+\/)*[^\s"'`<>)}\]/]+\.[a-zA-Z0-9]+)/g;
...
export function getAllChanges(): ChangeRecord[] {
	return Array.from(allChanges.values()).sort((a, b) => b.timestamp - a.timestamp);
}
...
export function setupRecorder(pi: ExtensionAPI): void {
	...
	pi.on('tool_call', async (event: { toolName: string; input: Record<string, unknown> }) => {
		...
		const DIRECT_PATH_KEYS = ['path', 'filePath', 'filepath', 'fileName', 'filename'] as const;

/**
 * Unified Edit Extension — 替代内置 edit 工具
 *
 * ⚠ 待观察：本文件从上游 mitsuhiko/agent-stuff 引入（commit 274fe04），
 * 属于实验性功能，未替代现有的 multi-edit.ts。
 * 引入目的是评估设计思路，未来根据稳定性决定是否迁移。
 *
 * 上游源文件：https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/unified-edit.ts
 *
 * 核心设计：
 * - 接受单一文本载荷（行编辑脚本或 Codex/apply_patch 补丁）
 * - 行操作：@REPLACE、@INS.PRE/POST、@INS.BEFORE/AFTER、@APPEND、@DEL
 * - 内联了 Pi 的模糊编辑匹配器（因非公开 API 而复制）
 * - 使用 pi 导出的 generateDiffString / generateUnifiedPatch 做 diff 渲染
 *
...
const log = createLogger('unified-edit');
...
function parseRowScript(text: string): RawFileScript[] {
	...
	function requireFile(lineNumber: number): RawFileScript {
		if (!currentFile)
			throw new Error(
				`Line ${lineNumber}: expected a [filename] header before operations or rows.`,
			);
		return currentFile;
	}
	...
	function pushGroup(marker: RowGroup['marker'], linesToAdd: string[]): void {
		if (!currentOp || !('groups' in currentOp))
			throw new Error('Internal parser error: group row without group operation.');
		if (marker === '@@') {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (raw.startsWith('@')) {
			...
			if (insertMatch) {
				...
				if (!Number.isSafeInteger(line) || line < 1)
					throw new Error(`Line ${lineNumber}: insert line number must be >= 1.`);
				currentOp =
			...
			if (delMatch) {
				...
				if (startLine < 1 || endLine < startLine)
					throw new Error(
						`Line ${lineNumber}: invalid inclusive delete range ${trimmed}.`,
					);
			...
			throw new Error(
				`Line ${lineNumber}: unknown edit operation ${trimmed}. Expected @INS.PRE, @INS.POST, @INS.BEFORE, @INS.AFTER, @REPLACE, @APPEND, or @DEL.`,
			);
		}
		...
		if (raw.startsWith('+') || raw.startsWith('-')) {
			...
			if (!('groups' in currentOp))
				throw new Error(`Line ${lineNumber}: unexpected row for @DEL.`);
		...
		throw new Error(
			`Line ${lineNumber}: invalid row script line. Every non-empty row must start with [filename], @, +, -, or a space-prefixed @REPLACE context row.`,
		);
	}
...
function getContextualReplacePairs(
	...
	for (let i = 0; i < hunks.length; i++) {
		...
		if (oldLines.length === 0) {
			throw new Error(
				`@REPLACE${label} in ${path} has + rows but no - or context rows to locate the insertion.`,
			);
		}
...
function getReplacePairs(
	...
	if (groups.some((group) => group.marker === ' ' || group.marker === '@@'))
		return getContextualReplacePairs(path, groups);
	...
	if (changeGroups.length % 2 !== 0) {
		throw new Error(
			`@REPLACE in ${path} has an odd number of +/- blocks. Pair each deleted block with an inserted block.`,
		);
	}
	...
	for (let i = 0; i < changeGroups.length; i += 2) {
		...
		if (a.marker === b.marker)
			throw new Error(
				`@REPLACE in ${path} has two adjacent ${a.marker} blocks; expected paired + and - blocks.`,
			);
		pairs.push({
...
function rowEditFromPair(
	pair: { oldLines: string[]; newLines: string[] },
	deleteWholeRows: boolean,
): Edit {
...
function applyReplaceOperation(
	...
	try {
		return applyEditsToNormalizedContent(content, primaryEdits, path, {
		...
		return applyEditsToNormalizedContent(content, fallbackEdits, path, {
			requireWholeLines: true,
		}).newContent;
	}
}
...
function applyAnchorInsertOperation(
	...
	if (groups.length !== 2 || groups[0].marker === groups[1].marker) {
		throw new Error(
			`${opName} in ${path} must contain exactly one - anchor block and one + insert block.`,
		);
	}
		...
		switch (op.kind) {
			...
			case 'insertAfter': {
				...
				if (index < 0 || index > doc.lines.length) {
					...
					throw new Error(
						`${opName} ${op.line} is outside ${path}; file has ${doc.lines.length} line(s).`,
					);
				}
...
async function buildRowPlan(text: string, cwd: string): Promise<ParsedPlan> {
	...
	return {
		mode: 'rows',
		changes: store.collectChanges('The row edit script produced no changes.'),
	};
}
...
function seekSequence(
	...
	const searchStart =
		eof && lines.length >= pattern.length ? lines.length - pattern.length : Math.max(0, start);
...
async function buildPatchPlan(text: string, cwd: string): Promise<ParsedPlan> {
	...
	const store = createSnapshotStore(cwd, (_path, absolutePath) =>
		maybeReadNormalized(absolutePath),
	);
...
function combineDetails(files: UnifiedEditDetails['files']): UnifiedEditDetails {
	...
	const firstChangedLine = files.find((file) => file.details.firstChangedLine !== undefined)
...
function previewForPlan(plan: ParsedPlan): Preview {
	...
	return {
		diff: details.diff,
		files: uniquePaths(plan.changes.map((change) => change.path)),
...
function getRenderablePaths(text: string | undefined): string[] | undefined {
	...
	try {
		const paths = patchLike
			? parsePatch(isPatchPayload(text) ? text : patchTextForPreview(text)).map(
					(op) => op.path,
				)
...
function requestUnifiedEditPreview(
	...
	const hasUsablePreview =
		component.preview && (!argsComplete || component.previewBuiltFromCompleteArgs);
	if (!text || !argsKey || hasUsablePreview || component.previewPendingArgsKey === argsKey)
		return;
...
export default function unifiedEditExtension(pi: ExtensionAPI) {
	log.info('Extension loaded — unified-edit tool registered (experimental, upstream)');
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params: UnifiedEditParams, signal, _onUpdate, ctx) {
			...
			log.info('Executing unified edit', { textLength: text.length, cwd: ctx.cwd });
			const plan = await buildPlan(text, ctx.cwd);
			log.debug('Plan built', { fileCount: plan.changes.length });
			try {
				await preflightPlan(plan, signal);
				log.debug('Preflight passed');
			} catch (err: any) {
				log.error('Preflight failed', { error: err?.message ?? String(err) });
				throw new Error(
		...
		renderCall(args, theme, context: RenderContext<UnifiedRenderState>) {
			const component = getUnifiedEditCallRenderComponent(
				context.state,
			...
			requestUnifiedEditPreview(component, text, key, context.cwd, context.argsComplete, () =>
				context.invalidate(),
			);
		...
		renderResult(result, _options, theme, context: RenderContext<UnifiedRenderState>) {
			...
			const output = formatUnifiedEditResult(
				component?.preview,
				typed,
				theme,
				context.isError,
